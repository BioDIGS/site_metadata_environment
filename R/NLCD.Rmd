...
```{r}
full_data_nlcd <- dplyr::left_join(full_data, dplyr::distinct(science_tree_canopy_cover_2023), by = c("lat", "lon"))
#full_data_nlcd <- dplyr::left_join(full_data_nlcd, dplyr::distinct(DF), by = c("lat", "lon"))

readr::write_csv(full_data_nlcd, "data/nlcd.csv")
```
