...
Diurnal Temperature Range (Celcius) - dtr: https://crudata.uea.ac.uk/cru/data/hrg/cru_ts_4.09/cruts.2503051245.v4.09/dtr/
Wet day frequency (days) - wet: https://crudata.uea.ac.uk/cru/data/hrg/cru_ts_4.09/cruts.2503051245.v4.09/wet/
Ground frost frequency (days) - frs: https://crudata.uea.ac.uk/cru/data/hrg/cru_ts_4.09/cruts.2503051245.v4.09/frs/
potential evapotranspiration (mm/day) - pet: https://crudata.uea.ac.uk/cru/data/hrg/cru_ts_4.09/cruts.2503051245.v4.09/pet/
...
data_source <- "google"
is_2025_available <- FALSE
```

```{r}
# Get GPS points and collection dates
# Locate data according to param above
if (data_source == "local") {
  # Look in pwd for specific snapshots
  soil_data <- read_csv("BioDIGS_soil_20250929.csv") %>% distinct(site_id, sample_id, collection_date)
  sites_data <- read.csv("BioDIGS Sample Data and Kit Request MASTER - ALL_SAMPLES - SITE DATA.csv") %>% select(site_id, gps)
} else if (data_source == "google") {
  # All from Google (need access)
  googlesheets4::gs4_auth(email = T)
  sites_data <- googlesheets4::read_sheet(
    "https://docs.google.com/spreadsheets/d/1Mbuym6GgG2B3VFCbjhYy4vaPCx1_tIOBlBOlT1iIHZE/edit?gid=476949426#gid=476949426",
    sheet = "SITE DATA"
  ) %>% select(site_id, gps)
  soil_data <- googlesheets4::read_sheet(
    "https://docs.google.com/spreadsheets/d/1Mbuym6GgG2B3VFCbjhYy4vaPCx1_tIOBlBOlT1iIHZE/edit?gid=476949426#gid=476949426",
    sheet = "SOIL SAMPLES",
    col_types = "c"
  ) %>% distinct(site_id, sample_id, collection_date)
} else {
  # Public snapshot on github if not downloaded
  sites_data <- read_csv(
    "https://github.com/FredHutch/GDSCNsoilsites/raw/refs/heads/main/data/snapshots/BioDIGS_sites_20260310.csv"
  ) %>% select(site_id, gps)
  soil_data <- read_csv(
    "https://github.com/FredHutch/GDSCNsoilsites/raw/refs/heads/main/data/snapshots/BioDIGS_soil_20260310.csv"
  ) %>% distinct(site_id, sample_id, collection_date)
}
```
...
**The following is a correction for 2025 dates not available. Will put them a year behind (e.g. sampling on 2025-09-01 becomes 2024-09-01, because that's likely to be most realistic climate-wise.**

```{r}
if(!is_2025_available){
 soil_dates <- 
  soil_dates %>% 
  mutate(collection_date = case_when(
    collection_date > as.Date("2025-01-01") ~ collection_date - period(years = 1), # Subtract a year from the sampling date
...
get_closest_point <- function(labels, latlon, date_, cru_dat, var_) {
  ...
  if (is.data.frame(latlon)) {
    ...
    if ("lat" %in% colnames(latlon) &
      ...
      # TODO: since 2025 data is not available, would be nice to filter out 2025 sampling dates and set them to NA.
      # It will automatically go to December 2024 which is the most recent, but not always correct.
      if (is.na(latlon$lat) | is.na(latlon$lon)) {
        ...
        var_final <- cbind(labels, latlon, date_, var_final)[1,] # Take first row if ties
...
```{r}
dtr_dat <- get_cru_dat(file_dir = "cru_ts/cru_ts4.09.2021.2024.dtr.dat.nc", var_ = "dtr")
dtr_out <- pmap_dfr(
  soil_dates,
  ~ get_closest_point(
    labels = data.frame(site_id = ..1, sample_id = ..2),
...
write_csv(dtr_out, "data/cru_dtr.csv")
```

Potential Evapotranspiration:

```{r}
pet_dat <- get_cru_dat(file_dir = "cru_ts/cru_ts4.09.2021.2024.pet.dat.nc", var_ = "pet")
pet_out <- pmap_dfr(
  soil_dates,
  ~ get_closest_point(
    labels = data.frame(site_id = ..1, sample_id = ..2),
...
write_csv(pet_out, "data/cru_pet.csv")
```

Number of wet days:

```{r}
wet_dat <- get_cru_dat(file_dir = "cru_ts/cru_ts4.09.2021.2024.wet.dat.nc", var_ = "wet")
wet_out <- pmap_dfr(
  soil_dates,
  ~ get_closest_point(
    labels = data.frame(site_id = ..1, sample_id = ..2),
...
write_csv(wet_out, "data/cru_wet.csv")
```